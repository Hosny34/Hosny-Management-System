# -*- coding: utf-8 -*-
"""Hourly local SQLite backups for Hosny POS.

Uses SQLite's online backup API so the live WAL database can be copied while
cashiers keep working.
"""
...
def prune_backups(db_path: str, backup_root: Optional[str] = None) -> None:
    ...
    for path in root.glob("warehouse_data-*.sqlite3*"):
        try:
            ...
            LOG.exception("failed to prune POS DB backup: %s", path)
    ...
    def _pump_queue(self) -> None:
        try:
            while True:
                ...
                if msg[0] == "ok":
                    LOG.info("POS DB hourly backup created: %s (%.1fs)", msg[1], msg[2])
                else:
                    LOG.error("POS DB hourly backup failed: %s", msg[1])
        except queue.Empty:
            pass
        try:
