...
def _updater_supports_gui(updater_script: str) -> bool:
    try:
        with open(updater_script, "r", encoding="utf-8", errors="ignore") as f:
            return "--gui" in f.read(200000)
    except Exception:
        return False
...
def launch_updater(download_result: Dict[str, Any], *, restart: bool = True) -> Dict[str, Any]:
    ...
    if restart:
        cmd.append("--restart")
    if _updater_supports_gui(updater_script):
        cmd.append("--gui")
