...
%PY_CMD% "verify_pe_arch.py" "dist\HosnyPOS.exe" x86
...
:try_python
set "CANDIDATE=%~1"
%CANDIDATE% -c "import sys, tkinter; ok=sys.version_info[:3]>=(3,7,3) and sys.version_info[:2]==(3,7); raise SystemExit(0 if ok else 1)" >nul 2>&1
if not errorlevel 1 (
