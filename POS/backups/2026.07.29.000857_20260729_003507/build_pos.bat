...
if /I "%HOSNY_AUTO_UPDATE%"=="1" if exist "prebuilt_update.json" if exist "HosnyPOS.exe" (
    echo [0/2] Checking packaged HosnyPOS.exe...
    "HosnyPOS.exe" --smoke-import >nul 2>&1
    if not errorlevel 1 (
        if not exist "dist" mkdir "dist"
        copy /Y "HosnyPOS.exe" "dist\HosnyPOS.exe" >nul
