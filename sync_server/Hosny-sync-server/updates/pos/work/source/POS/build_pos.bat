...
set "PREBUILT_EXE="
if exist "prebuilt\HosnyPOS.exe" set "PREBUILT_EXE=prebuilt\HosnyPOS.exe"
if not defined PREBUILT_EXE if exist "HosnyPOS.exe" set "PREBUILT_EXE=HosnyPOS.exe"

if /I "%HOSNY_AUTO_UPDATE%"=="1" if exist "prebuilt_update.json" if defined PREBUILT_EXE (
    echo [0/2] Checking packaged HosnyPOS.exe...
    "!PREBUILT_EXE!" --smoke-import >nul 2>&1
    if not errorlevel 1 (
        if not exist "dist" mkdir "dist"
        copy /Y "!PREBUILT_EXE!" "dist\HosnyPOS.exe" >nul
        if exist "cacert.pem" copy /Y "cacert.pem" "dist\cacert.pem" >nul
        echo Packaged executable verified and staged.
        exit /b 0
    )
    echo Packaged executable did not pass smoke test. Falling back to local build.
)
